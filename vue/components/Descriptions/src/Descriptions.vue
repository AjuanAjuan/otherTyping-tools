...
const mobile = computed(() => globalStore.isMobile) //computed(() => appStore.getMobile)
...
const prefixCls = 'descriptions' // getPrefixCls('descriptions')
...
<template>
	<div :class="[
		...
		<div v-if="title" :class="[
			...
			<div :class="[`${prefixCls}-header__title`, 'relative text-17px font-bold ml-10px']">
		...
		<ElCollapseTransition>
			<div v-show="show" :class="[`${prefixCls}-content`, 'p-10px']">
				<ElDescriptions :column="mobile ? 1 : 2" border :direction="mobile ? 'vertical' : 'horizontal'"
					v-bind="getBindValue">
					...
					<ElDescriptionsItem v-for="item in schema" :key="item.field" v-bind="getBindItemValue(item)">
						...
						<template #default>
							<slot :name="item.field" :row="data">
								<component v-if="item.formatter"
									:is="item.formatter({ record: data, schema: item, value: data[item.field] })">
								</component>
								<template v-else>
									{{ data[item.field] }}
								</template>
...
<style>
.descriptions-header__title::after {
	position: absolute;
	top: 2px;
	left: -10px;
	width: 4px;
